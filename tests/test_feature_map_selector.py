...
def test_non_adaptive_target_path_gives_exact_output_dim(Cls, data):
    X, y = data
    t = Cls(output_dim=6, target_aware=True, task="regression").fit(X, y)
...
def test_adaptive_target_path_clamps_within_window(Cls, data):
    X, y = data
    t = Cls(output_dim=8, target_aware=True, adaptive=True, min_output_dim=3, max_output_dim=6).fit(X, y)
...
def test_default_selector_center_snapshot(data):
    ...
    X, y = data
    t = RBFExpansionTransformer(output_dim=5, target_aware=True, task="regression").fit(X, y)
...
def test_invalid_selector_raises(data):
    X, y = data
    with pytest.raises(InvalidParamError, match="placement_strategy must be 'cart' or 'lightgbm'"):
        RBFExpansionTransformer(target_aware=True, placement_strategy="bogus").fit(X, y)
...
def test_selector_is_a_clonable_hyperparameter():
    t = RBFExpansionTransformer(placement_strategy="lightgbm")
